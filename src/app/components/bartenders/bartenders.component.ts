...
export class BartendersComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    interval(1000)
      ...
      .subscribe(
        (data) => {
          this.orders = this.sortOrders((data as any).data.map((order: apiData.DrinkOrder) => ({_id: order._id, cod: order.cod, table: order.table, ready: order.ready, drinks: order.drinks, date: new Date(order.date),})));
        },
        (error) => {
          console.error('Error fetching data:', error);
        }
